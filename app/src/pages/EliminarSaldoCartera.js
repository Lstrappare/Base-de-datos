...
const EliminarSaldoCartera = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.delete(`http://localhost:3001/api/eliminar-saldo-cartera`, {
        data: { carteraID, criptomonedaID }
      });
      setMensaje('Saldo de la cartera eliminado exitosamente');
    } catch (error) {
      console.error('Error eliminando saldo de la cartera', error);
      setMensaje('Error al eliminar el saldo de la cartera');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center">
      <div className="bg-white shadow-lg rounded-lg p-8 max-w-md w-full">
        <h1 className="text-2xl font-bold text-center mb-6 text-gray-800">Eliminar Saldo de Cartera</h1>
...
export default EliminarSaldoCartera;
